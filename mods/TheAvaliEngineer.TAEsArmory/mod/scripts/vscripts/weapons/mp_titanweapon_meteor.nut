...
void function Scorch_SelfDamageReduction( entity target, var damageInfo ) {
	...
	if( IsMultiplayer() ) {
		...
		//	Has tempered plating
		if( SoulHasPassive( soul, ePassives.PAS_SCORCH_SELFDMG ) ) {
			//	Scale
			DamageInfo_ScaleDamage( damageInfo, SELFDAMAGE_SCALE_MOD )

			//	Add damage resist
			int statusEffectID = StatusEffect_AddTimed( soul, eStatusEffect.damage_reduction, MOD_DAMAGE_REDUCTION, MOD_EFFECT_TIME, MOD_EFFECT_FADE_TIME )
			//print("[TAEsArmory] TemperedPlating_OnSelfDamage: status effect ID = " + statusEffectID)

			return
		}

		//	No tempered plating
		DamageInfo_ScaleDamage( damageInfo, SELFDAMAGE_SCALE_DEF )
		return
	}

	//	Singleplayer
	DamageInfo_ScaleDamage( damageInfo, SELFDAMAGE_SCALE_SP )
}
